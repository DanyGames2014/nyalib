...
public class FlattenedChunkMixin {
    ...
    public boolean dontCallOnBreak(Block instance, World x, int y, int z, int i, @Local(ordinal = 0, argsOnly = true) BlockState state) {
        if (instance instanceof NetworkComponent && state.getBlock() instanceof NetworkComponent) {
            return false;
        }
        return true;
    }
