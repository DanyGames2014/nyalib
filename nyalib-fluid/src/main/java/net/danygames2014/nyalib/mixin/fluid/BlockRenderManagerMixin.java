...
public abstract class BlockRenderManagerMixin {
    ...
    @ModifyExpressionValue(method = "renderFluid", at = @At(value = "CONSTANT", args = "floatValue=8.0", ordinal = 0))
    public float a(float original, @Local(ordinal = 0, argsOnly = true) int x, @Local(ordinal = 1, argsOnly = true) int y, @Local(ordinal = 2, argsOnly = true) int z) {
        if (this.blockView.getBlockMeta(x, y, z) == 0) {
            return 8.0F;
        }
        return original;
    }

    @ModifyExpressionValue(method = "renderFluid", at = @At(value = "CONSTANT", args = "floatValue=8.0", ordinal = 1))
    public float b(float original, @Local(ordinal = 0, argsOnly = true) int x, @Local(ordinal = 1, argsOnly = true) int y, @Local(ordinal = 2, argsOnly = true) int z) {
        if (this.blockView.getBlockMeta(x, y, z) == 0) {
            return 8.0F;
        }
        return original;
